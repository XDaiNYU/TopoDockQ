...
def main():
    ...
    X_single=df3
    X_single['pdb2sql_DockQ'] = 1.0 # Dummy value added to match training column requirements; does not affect inference output.
    # Split data by class
    ...
    X_single=X_single[valid_columns]
    ...
    X_single, y_single=get_features_and_target(X_single)
    ...
    single_data=TensorDataset(torch.tensor(X_single, dtype=torch.float32), torch.tensor(y_single, dtype=torch.float32))
    
    
    train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_data, batch_size=1000000)
    test_loader = DataLoader(test_data, batch_size=1000000)
    single_loader = DataLoader(single_data, batch_size=1000000)
    # Set up device
    ...
    # Perform inference on the example case
    print("Performing inference on singe set...")
    perform_inference(best_model, single_loader, device, "example_inference_results.csv")
